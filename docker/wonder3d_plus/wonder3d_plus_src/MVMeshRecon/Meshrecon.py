...
from MVMeshRecon.MeshRecon.util.func import save_obj, save_images_init
from MVMeshRecon.MeshRecon.remesh import calc_vertex_normals
import torch
from MVMeshRecon.MeshRecon.optimize_remesh import optimize_remesh
...
def Meshrecon(mv_normal, proj_normal, gt_normals, mv_RGB, proj_RGB, gt_RGBs, RGB_refine_index=None, RGB_view_weights=None, vertices_init=None, faces_init=None, debug_path = None, clean_mesh=True, persp=True, output_path='debug'):
    ...
    vertices, faces = optimize_remesh(vertices_init, faces_init, gt_normals_world, renderer, None, edge_len_lims=(0.01, 0.05), remeshing_steps=200)
    ...
    meshes = simple_clean_mesh(transfer_to_pyml_mesh(vertices, faces), apply_smooth=True, stepsmoothnum=1,
    ...
    textured_mesh = warpper(vertices, faces, gt_RGBs, mv_RGB, proj_RGB, weights=RGB_view_weights, refine_index=RGB_refine_index, render_size=gt_RGBs.shape[-2], b_perspective=persp, visualize=True, do_refine_uv=False)
