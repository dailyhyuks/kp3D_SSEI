...
from pytorch3d.renderer.cameras import look_at_view_transform, OrthographicCameras, CamerasBase, PerspectiveCameras
...
def get_camera(world_to_cam, fov_in_degrees=60, focal_length=35, cam_type='fov'):
    ...
    if cam_type == 'fov':
        camera = FoVPerspectiveCameras(device=world_to_cam.device, R=R, T=T, fov=fov_in_degrees, znear = 0.01, degrees=True)
    else:
...
class PixeltoFaceRenderer:
    ...
    def transform_vertices(self, meshes: Meshes, cameras: CamerasBase, opt_render = None):
        if opt_render:
            vertices = meshes.verts_padded()
            vert_hom = torch.cat([vertices, torch.ones_like(vertices[..., :1])],axis=-1) #V,3 -> V,4
            vertices = vert_hom @ opt_render._mvp[cameras.idx:cameras.idx+1].transpose(-2,-1) #C,V,4
            # vertices = vertices[...,:3]
            
            # vertices_raw = cameras.transform_points_ndc(meshes.verts_padded())
    ...
    def render_pix2faces_nvdiff(self, meshes: Meshes, cameras: CamerasBase, H=512, W=512, opt_render = None):
        meshes = meshes.to(self.device)
        cameras = cameras.to(self.device)
        vertices = self.transform_vertices(meshes, cameras, opt_render = opt_render)
        ...
        # import torchvision
        # torchvision.utils.save_image(pix_to_face.float(),'rast.png')
        # exit()
        return pix_to_face
...
def get_visible_faces(meshes: Meshes, cameras: CamerasBase, resolution=1024, opt_render = None):
    ...
    pix_to_face = pix2faces_renderer.render_pix2faces_nvdiff(meshes, cameras, H=resolution, W=resolution, opt_render = opt_render)

    unique_faces = torch.unique(pix_to_face.flatten())
    unique_faces = unique_faces[unique_faces != -1]
    return unique_faces

def project_color(meshes: Meshes, cameras: CamerasBase, pil_image , use_alpha=True, eps=0.05, resolution=1024, device="cuda", opt_render = None) -> dict:
    """
    ...
    image = pil_image.permute(2,0,1).float().to(device) #torch.from_numpy(np.array(pil_image.convert("RGBA")) / 255.).permute((2, 0, 1)).float().to(device)     # in CHW format of [0, 1.]
    unique_faces = get_visible_faces(meshes, cameras, resolution=resolution, opt_render = opt_render)
    ...
    if opt_render:
        ...
        #vertices = vert_hom @ opt_render._mvp[cameras.idx:cameras.idx+1].transpose(-2,-1) #C,V,4
        ndc_points = torch.tensor([[[0., 0., 0.1, 1.0], [0., 0., 0.2, 1.0]]]).to(device)
        world_points_render = ndc_points @ torch.inverse(opt_render._mvp[cameras.idx:cameras.idx+1].transpose(-2,-1))
        world_points_render = world_points_render[...,:3] / world_points_render[...,3:]
        world_points_render = world_points_render[0][:,:3]
    else:    
        world_points = cameras.unproject_points(torch.tensor([[[0., 0., 0.1], [0., 0., 0.2]]]).to(device))[0]

    view_direction = world_points[0] - world_points[1]
    ...
    # with open('visi.xyz','w') as f:
    #     for i in range(verts_coordinates.shape[0]):
    #         f.write('%f %f %f\n'%(verts_coordinates[i,0],verts_coordinates[i,1],verts_coordinates[i,2]))
    # exit()
    # compute color
    
    if opt_render:
        vert_hom = torch.cat([verts_coordinates, torch.ones_like(verts_coordinates[..., :1])],axis=-1) #V,3 -> V,4
        pt_tensor = vert_hom @ opt_render._mvp[cameras.idx:cameras.idx+1].transpose(-2,-1) #C,V,4
        pt_tensor = pt_tensor[0][...,:2] / pt_tensor[0][...,3:]
    else:   
    ...
    # debug_pos = (verts_coordinates[valid]).detach().cpu().numpy()
    # valid_pt_debug = ((valid_pt.detach().cpu().numpy() + 1) * 256).astype(np.uint32)
    # for debug_pt in valid_pt_debug:
    #     img_np[debug_pt[1],debug_pt[0],:3] = 1
    ...
    # debug_color = valid_color.detach().cpu().numpy()
    # with open('color_pts.xyz','w') as f:
    #     for point, color in zip(debug_pos,debug_color):
    ...
    cos_angles = (valid_verts_normals * view_direction).sum(dim=1)   # [-1, 0]
    return {
...
def multiview_color_projection(meshes: Meshes, image_list: List[Image.Image], cameras_list: List[CamerasBase]=None, camera_focal: float = 35, weights=None, eps=0.05, resolution=512, device="cuda", reweight_with_cosangle="square", use_alpha=True, confidence_threshold=0.1, complete_unseen=False, below_confidence_strategy="smooth", opt_render = None) -> Meshes:
    """
    Projects color from a given image onto a 3D mesh.
    ...
    for camera, image, weight in zip(cameras_list, image_list, weights):
        ret = project_color(meshes, camera, image, eps=eps, resolution=resolution, device=device, use_alpha=use_alpha, opt_render = opt_render)
...
def get_cameras_list(azim_list, device, focal=2/1.35, dist=1.1):
    ...
    for azim in azim_list:
        ...
        cameras: PerspectiveCameras = get_camera(w2c, focal_length=focal, cam_type='orthogonal').to(device)
        ret.append(cameras)
    return ret

def get_cameras_list_py3d(extrinsic_list, intrinsic_list, imres, device, b_perspective=False):
    ret = []
    i=0
    for extrinsic, intrinsic in zip(extrinsic_list,intrinsic_list):
        R, T = extrinsic[:3,:3] , extrinsic[:3,3] #look_at_view_transform(dist, 0, azim)
        w2c = torch.cat([R, T[:, None]], dim=1)
        fov = 2*np.arctan(32./70)

        fov_in_degree = 49.13
        if b_perspective:
            cameras: PerspectiveCameras = get_camera(w2c, fov_in_degrees = fov_in_degree, cam_type='fov').to(device)
        else:
            cameras: OrthographicCameras = get_camera(w2c, fov_in_degrees = fov_in_degree, cam_type='Ortho').to(device)
        cameras.idx = i
        i+=1
        ret.append(cameras)
    return ret
